...
ECAL::ECAL() : // clang-format off
    ...
    fMPPCEfficiency{0.038361565, 0.043881036, 0.050158203, 0.056879188, 0.063896051, 0.071245776, 0.077096023, 0.082806497, 0.09049302,
                    ...
                    0.034501313},
    fWaveformIntegralTime{fScintillationTimeConstant1 * 7},
    fEnergyFraming{},
...
auto ECAL::ComputeMesh() const -> MeshInformation {
    ...
    Mustard::PrintLn<'I'>("===========================");
    Mustard::PrintLn<'I'>("Information for ECAL\n");
    Mustard::PrintLn<'I'>(">>> Module Sorting of ECAL");
    Mustard::PrintLn<'I'>("===========================");
    for (int type{1}; auto&& pair : edgeLengthSet) {
        std::ranges::sort(pair.second);
        Mustard::PrintLn<'I'>("--- Type {}: \n", type);
        Mustard::PrintLn<'I'>("- edge lengths: ");
        for (auto&& value : pair.first) {
            Mustard::Print<'I'>("{} ", value);
        }
        Mustard::PrintLn<'I'>("\n");
        Mustard::PrintLn<'I'>("- module IDs ({} modules total): ", pair.second.size());
    ...
    for (auto&& pair : edgeLengthSet) {
        for (auto&& value : pair.second) {
            typeMap[value] = typeID; // moduleID->typeID
        }
        typeID++;
    }
